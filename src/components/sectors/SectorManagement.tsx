...
const SectorManagement = () => {
  ...
  const { sectors, leaders, sectorLeaderAssignments, loading, error, refresh } = useSupabaseData([
    "sectors",
    "leaders",
  ...
  const canDeleteSectors = canDeleteAdminRecords();
  ...
  const handleDelete = async (sectorId: string) => {
    if (!canDeleteSectors) {
      toast({
        ...
        description: "Somente ADM pode excluir setores.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      console.error("Erro ao remover setor:", deleteError);
      toast({
        title: "Erro",
  ...
  if (error) {
    return <div className="rounded border border-destructive/20 bg-destructive/10 p-4 text-destructive">Erro ao carregar setores: {error}</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie os setores e atribua líderes responsáveis</p>
        </div>
      ...
      {sectors.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="mb-2 text-lg font-semibold">Nenhum setor cadastrado</h3>
            <p className="mb-4 text-center text-muted-foreground">Comece adicionando o primeiro setor da sua empresa</p>
            <AddSectorDialog onSectorAdded={refresh} leaders={leaders} />
          </CardContent>
        </Card>
      ) : (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Total de {sectors.length} setores cadastrados</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {sectors.map((sector) => (
                  <TableRow key={sector.id}>
                    ...
                    <TableCell>
                      {sector.description || <span className="italic text-muted-foreground">Sem descrição</span>}
                    </TableCell>
                    <TableCell>
                      {(() => {
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleDelete(sector.id)}>Excluir</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        )}
