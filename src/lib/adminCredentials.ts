...
const HIDDEN_ODAIR_USERNAME_PARTS = ["odair"];
...
const isHiddenOdairUsername = (username: string) =>
  HIDDEN_ODAIR_USERNAME_PARTS.some((part) => normalizeUsername(username).includes(part));
    ...
    return data
      .filter(
        (item) =>
          ...
          !isHiddenOdairUsername(item.username),
      )
      .map((item) => ({
...
export const upsertInvestigatorAccount = async (
  ...
  if (isHiddenOdairUsername(normalized)) return false;
