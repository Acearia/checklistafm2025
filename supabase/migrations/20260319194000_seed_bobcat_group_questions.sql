DO $$
DECLARE
  empilhadeira_group_id uuid;
BEGIN
  SELECT id
    INTO empilhadeira_group_id
  FROM public.checklist_groups
  WHERE lower(name) = 'empilhadeira'
  ORDER BY created_at
  LIMIT 1;

  IF empilhadeira_group_id IS NULL THEN
    ...
    RETURNING id INTO empilhadeira_group_id;
  END IF;

  INSERT INTO public.group_questions (group_id, question, alert_on_yes, alert_on_no, order_number)
  SELECT empilhadeira_group_id, v.question, v.alert_on_yes, v.alert_on_no, v.order_number
  FROM (
    VALUES
      ...
      ('O sistema hidráulico (mangueiras e bomba) apresenta algum aspecto que indique vazamento de óleo?', true, false, 6),
      ('O sistema de frenagem, testado pelo operador no momento da inspeção, apresenta algum problema?', true, false, 7),
  ...
  WHERE e.kp = '1239'
    AND NOT EXISTS (
      ...
      WHERE eg.equipment_id = e.id
        AND eg.group_id = empilhadeira_group_id
    );
END $$;
